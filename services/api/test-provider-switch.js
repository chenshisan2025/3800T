...
async function testEndpointProvider(url, expectedProvider) {
  try {
    ...
    if (actualProvider === expectedProvider) {
      ...
      log('red', `❌ ${url} - Expected: ${expectedProvider}, Got: ${actualProvider}`);
      return false;
    }
  } catch (error) {
...
function runTestWithProvider(providerType, expectedProvider) {
  return new Promise((resolve) => {
    ...
    // 设置环境变量并启动服务器
    const env = { ...process.env, DATA_PROVIDER_TYPE: providerType, PORT: '3002' };
        ...
        if (!serverReady) {
          ...
          setTimeout(async () => {
            ...
            for (const endpoint of endpoints) {
              const result = await testEndpointProvider(endpoint, expectedProvider);
...
async function runProviderSwitchTest() {
  ...
  for (const testCase of testCases) {
    const result = await runTestWithProvider(testCase.providerType, testCase.expectedProvider);
    results.push(result);
  ...
  results.forEach(result => {
    ...
    log(statusColor, `${statusIcon} ${result.providerType.toUpperCase()} -> ${result.expectedProvider}`);
