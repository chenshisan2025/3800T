...
import { startMarketScanCron, stopMarketScanCron, cronScheduler, scanMarketAndCheckAlerts } from '@/lib/cron';
...
export async function GET(request: NextRequest) {
  try {
    ...
    logError(error as Error, 'GetCronStatus');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!action || !task) {
      return apiResponse.error('请提供 action 和 task 参数', 400, 'MISSING_PARAMS');
    }
    ...
    logError(error as Error, 'ManageCronTask');
