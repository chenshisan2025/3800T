...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      half_open_breakers: circuitBreakers.filter(cb => cb.state === 'half_open').length,
      closed_breakers: circuitBreakers.filter(cb => cb.state === 'closed').length,
      total_requests: circuitBreakers.reduce((sum, cb) => sum + cb.totalRequests, 0),
      total_failures: circuitBreakers.reduce((sum, cb) => sum + cb.failedRequests, 0),
      overall_success_rate: 0,
    };
    
    if (summary.total_requests > 0) {
      summary.overall_success_rate = 
        ((summary.total_requests - summary.total_failures) / summary.total_requests) * 100;
    }
    ...
    const result = {
      ...
      circuit_breakers: circuitBreakers.map(cb => ({
        ...
        last_failure_time: cb.lastFailureTime ? new Date(cb.lastFailureTime).toISOString() : null,
        last_success_time: cb.lastSuccessTime ? new Date(cb.lastSuccessTime).toISOString() : null,
        success_rate: cb.totalRequests > 0 ? 
          (cb.successfulRequests / cb.totalRequests) * 100 : 0,
        failure_rate: cb.totalRequests > 0 ? 
          (cb.failedRequests / cb.totalRequests) * 100 : 0,
      })),
  ...
  try {
    ...
    if (reset_all) {
      ...
      return apiResponse.success(
        { message: '所有熔断器已重置', reset_count: circuitBreakerManager.getCount() },
        '重置所有熔断器成功'
      );
      ...
      return apiResponse.error('请指定要重置的熔断器名称或设置 reset_all 为 true', 400);
    }
  } catch (error) {
    recordError(
