...
export async function GET(
  ...
  try {
    ...
    // 验证日期范围
    if (start_date && end_date && !dateUtils.isValidDateRange(start_date, end_date)) {
      return apiResponse.error('开始日期不能晚于结束日期', 400, 'INVALID_DATE_RANGE');
    }
    ...
    const stats = stockData.length > 0 ? {
      ...
      volume_stats: {
        ...
        avg: stockData.reduce((sum, d) => sum + d.volume, 0) / stockData.length,
      },
    } : null;
...
export async function POST(
  ...
  try {
    ...
    // 验证必需字段
    const requiredFields = ['date', 'open_price', 'close_price', 'high_price', 'low_price', 'volume'];
    for (const field of requiredFields) {
      if (body[field] === undefined || body[field] === null) {
        return apiResponse.error(`缺少必需字段: ${field}`, 400, 'MISSING_REQUIRED_FIELD');
      }
    }
    ...
    if (high_price < Math.max(open_price, close_price, low_price)) {
      return apiResponse.error('最高价不能低于开盘价、收盘价或最低价', 400, 'INVALID_PRICE_DATA');
    }
    
    if (low_price > Math.min(open_price, close_price, high_price)) {
      return apiResponse.error('最低价不能高于开盘价、收盘价或最高价', 400, 'INVALID_PRICE_DATA');
    }
    ...
    // 计算涨跌幅
    const change_percent = body.change_percent || 
