...
import { getDatabaseBreaker, getAIServiceBreaker, getRateLimitBreaker } from '@/lib/circuitBreaker';
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (hasSimplified || timeoutNotices.length > 0) {
          degradationNotice = timeoutNotices.length > 0 
            ? timeoutNotices[0] 
            : 'AI分析服务部分功能降级，已提供简化版分析结果。';
        }
        
      } catch (aiError) {
        const isCircuitBreakerOpen = aiError instanceof Error && aiError.message.includes('Circuit breaker');
        
        recordError(
          aiError instanceof Error ? aiError : 'AI analysis failed or circuit breaker open',
      ...
      const isNotFound = dbError instanceof Error && dbError.message === '股票不存在';
    ...
    if (stockData.length > 0) {
      ...
      technicalIndicators = {
        ...
        avg_volume_5d: volumes.slice(0, 5).reduce((a, b) => a + b, 0) / Math.min(5, volumes.length),
        avg_price_5d: prices.slice(0, 5).reduce((a, b) => a + b, 0) / Math.min(5, prices.length),
        avg_price_20d: prices.slice(0, 20).reduce((a, b) => a + b, 0) / Math.min(20, prices.length),
