...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (authError) {
      logger.error('登录认证失败', { error: authError.message });
    ...
    try {
      ...
      if (!user) {
        ...
        logger.info(`为现有 Auth 用户创建数据库记录: ${authData.user.email}`, {
          userId: user.id,
        });
      } else {
        // 更新最后登录时间
      ...
      logger.info(`用户登录成功: ${email}`, { userId: user.id });
      ...
      logger.error('登录时数据库操作失败', dbError);
