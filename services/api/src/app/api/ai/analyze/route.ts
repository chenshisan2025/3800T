...
const AIAnalysisQuerySchema = z.object({
  ...
  analysis_type: z
    .enum(['comprehensive', 'fundamental', 'technical', 'sentiment', 'risk'])
    .default('comprehensive'),
  use_llm: z
    .string()
    .transform(val => val === 'true')
    .default('false'), // URL参数转布尔值
...
export interface StandardAIAnalysisResult {
  ...
  cites: string[]; // 数据来源引用
  meta: {
    timeliness: 'real-time' | 'delayed' | 'historical';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 检查AI报告使用限制
    const dailyUsage = await getCurrentUsage(
      user.id,
      FeatureType.AI_REPORTS_DAILY
    );
    const monthlyUsage = await getCurrentUsage(
      user.id,
      FeatureType.AI_REPORTS_MONTHLY
    );
    ...
    const standardResult: StandardAIAnalysisResult = {
      ...
      cites: result.metadata.data_sources,
      meta: {
        timeliness: result.metadata.provider_info.isPrimary
          ? 'real-time'
          : 'delayed',
        analysis_duration_ms: result.metadata.analysis_duration_ms,
        llm_enhanced: result.metadata.llm_enhanced,
        data_sources_count: result.metadata.data_sources.length,
      },
    };
