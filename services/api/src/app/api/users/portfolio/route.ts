...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.error('获取用户投资组合列表失败', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 检查投资组合数量限制
    const currentPortfolioCount = await getCurrentUsage(
      dbUser.id,
      FeatureType.PORTFOLIO_ITEMS
    );
