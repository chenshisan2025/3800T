...
export class SentimentAnalyzer {
  ...
  async analyze(
    ...
    try {
      ...
      if (!newsResponse.data || newsResponse.data.length === 0) {
        logger.warn('未找到相关新闻数据，使用默认情绪分析', { stock_code: stockCode });
      ...
      // 计算整体情绪得分
      const sentimentScore = this.calculateSentimentScore(newsSummary, newsData);
  ...
  private analyzeNewsSentiment(newsData: any[]) {
    ...
    newsData.forEach(news => {
      ...
      const sentiment = news.sentiment || this.analyzeTitleSentiment(news.title);
  ...
  private analyzeTitleSentiment(title: string): 'positive' | 'negative' | 'neutral' {
    const positiveWords = ['上涨', '增长', '突破', '利好', '看好', '乐观', '强势', '创新高', '盈利', '业绩'];
    const negativeWords = ['下跌', '下滑', '跌破', '利空', '看空', '悲观', '疲软', '创新低', '亏损', '风险'];
  ...
  private extractKeywords(title: string): string[] {
    const keywords = ['财报', '业绩', '重组', '并购', '分红', '增持', '减持', '回购', '投资', '合作'];
  ...
  private analyzeMarketMood(newsData: any[], sentimentScore: number) {
    ...
    // 社交热度（模拟）
    const socialBuzz = Math.abs(sentimentScore) > 0.5 ? 'strong' : 
                      Math.abs(sentimentScore) > 0.2 ? 'moderate' : 'weak';
  ...
  private extractKeyEvents(newsData: any[]): string[] {
    ...
    const importantNews = newsData
      .filter(news => {
        ...
        return title.includes('财报') || title.includes('业绩') || 
               title.includes('重组') || title.includes('并购') ||
               title.includes('分红') || title.includes('增持') ||
               title.includes('减持') || title.includes('回购');
      })
      .slice(0, 5); // 最多5个重要事件
  ...
  private generateTemplateAnalysis(
    ...
    const templates = this.templates[sentimentLabel as keyof typeof this.templates];
    ...
    // 基于情绪一致性调整信心度
    const dominantSentiment = Math.max(newsSummary.positive_count, newsSummary.negative_count, newsSummary.neutral_count);
  ...
  private async generateLLMAnalysis(
    ...
    const recentNews = newsData.slice(0, 10).map(news => `- ${news.title}`).join('\n');
...
请以JSON格式返回结果。`;
    ...
    try {
      ...
      // 回退到模板分析
      const sentimentLabel = this.getSentimentLabel(sentimentScore);
      return this.generateTemplateAnalysis(stockCode, newsData, sentimentLabel, newsSummary, marketMood);
    }
  }
   ...
   * 生成默认情绪分析（当没有新闻数据时）
   */
  private generateDefaultSentimentAnalysis(stockCode: string): SentimentAnalysisResult {
    return {
      analysis: '由于缺乏足够的新闻数据，当前无法准确评估市场情绪倾向，建议关注后续相关报道和市场动态情景。',
      sentiment_score: 0,
