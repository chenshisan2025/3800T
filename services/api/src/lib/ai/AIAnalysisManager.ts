...
import { getOrchestrator } from '@/lib/orchestrator/Orchestrator';
  ...
  async analyze(query: AIAnalysisQuery): Promise<AIAnalysisResult> {
    ...
    try {
      ...
      if (use_llm && llm_provider) {
        await this.orchestrator.initializeLLMProvider(llm_provider);
      }
      ...
      if (
        ...
        analysisResults.fundamental = await this.fundamentalAnalyzer.analyze(
          stock_code,
          undefined // LLM provider will be handled internally
        );
      ...
      analysisResults.metadata = {
        ...
        llm_enhanced: use_llm && !!llm_provider,
    ...
    try {
      ...
      const status = {
        ...
        features: {
          ...
          risk_analysis: true,
          llm_enhancement: Object.values(llmStatus).some(
      ...
      logger.info('AI分析服务状态检查完成', {
        ...
        analyzers_healthy: Object.values(analyzersStatus).every(
          status => status
        ),
