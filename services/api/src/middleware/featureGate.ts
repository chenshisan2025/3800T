    ...
    try {
      ...
      req.featureGate = {
        ...
        checkLimit: async (feature: FeatureType, currentCount?: number) => {
          const count =
            currentCount ?? (await getCurrentUsage(user.id, feature));
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!canUse) {
        ...
        const limit =
          featureGate.limits[feature as keyof typeof featureGate.limits];
...
export async function getUserFeatureInfo(userId: string) {
  ...
  // 获取当前使用量
  const [aiReportsDaily, aiReportsMonthly, watchlistCount, alertsCount] =
    await Promise.all([
