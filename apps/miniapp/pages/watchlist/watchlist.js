...
const { featureGate, checkLimit, showLimitPrompt, getUserSubscription } = require('../../utils/featureGate')
...
Page({
  ...
  async loadWatchlist() {
    try {
      ...
      if (response.success) {
        ...
        app.showToast(response.message || '加载失败', 'error')
      }
    } catch (error) {
  ...
  async onDeleteSelected() {
    ...
    try {
      const result = await app.showModal('确认删除', `确定要删除选中的 ${this.data.selectedStocks.length} 只股票吗？`)
  ...
  async onDeleteStock(e) {
    ...
    try {
      const result = await app.showModal('确认删除', `确定要删除 ${stock.name}(${stock.code}) 吗？`)
      if (!result.confirm) return
